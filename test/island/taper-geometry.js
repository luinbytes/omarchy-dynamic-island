...
for (const edge of ["top", "bottom", "left", "right"]) {
  ...
  let topology = null
  for (const depth of [68, 32, 24, 16, 10, 5, 2, 1]) {
    ...
    lastSpan = span
    assert.equal(outline.contentSafe, depth >= 16)
    ...
    const values = outline.perimeter.match(/-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?/g).map(Number)
  ...
  const before = sample(edge, 16.001)
  const after = sample(edge, 15.999)
